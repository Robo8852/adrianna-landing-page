...
3. **Rate limiting (server-side, authoritative)** — cap submissions per identifier/window in the Convex mutation. Convex offers a rate-limiter component (`@convex-dev/rate-limiter`) — prefer it over hand-rolled. Limit by a coarse key (e.g. normalized email, and/or a client-provided token) since Convex mutations don't expose raw client IP. ❓ *Open: what key + thresholds (see §10).*
...
3. **§6.3** Rate-limit key + thresholds — by email? by a client token? per-minute/hour/day caps? *(default: `@convex-dev/rate-limiter`, key by normalized email, conservative per-window cap TBD)*
